...
# 划分 训练集 和 测试集
X_train, X_finalTest, y_train, finalTest = train_test_split(df_loan, df_loan[['risk']], test_size=0.2, random_state=0)
...
for clf_name1, classifier1 in classifiers1.items():
    ''' 分类器1：划分"无风险"和"有风险" '''
    # 对classifier1的训练集再次划分测试集，用于单独统计正确率
    X1_train, X1_test, y1_train, y1_test = train_test_split(X_train.drop(columns=['label', 'risk']), y_train,
    ...
    for clf_name2, classifier2 in classifiers2.items():
        ...
        X_finalTest.drop(columns=['pred'], inplace=True)
        ...
        accuracy = accuracy_score(y_finalTest, finalRes['result'])
        title = f"{num}/25 {clf_name1}+{clf_name2}"
        num += 1
        print(f"{title} accuracy={accuracy}")
        with open("accuracy.txt", "a") as f_ac:
            f_ac.write(f"{title} accuracy={accuracy}\n")
        # 记录当前组合分类结果
        reports = {'title': title,
                   'classifier1': classification_report(y1_test, Y1_pred),
                   'classifier2': classification_report(y2_test, Y2_pred),
                   'final': classification_report(y_finalTest, finalRes['result'])
                   }
        outputContent.append(reports)

print('\n', outputContent)
# 将输出结果写入到 txt 文件中
with open("output.txt", "w") as f:
    f.write("分类器的预测报告：" + str(outputContent))
